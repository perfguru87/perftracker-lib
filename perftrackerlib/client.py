...
class ptSuite:
    ...
    def upload(self):
        ...
        if len(self.env_nodes) == 0:
            msg = "Interrupt tests upload() because environment nodes are not specified"
            if self._pt_options_added:
                logging.warning("%s, either use --pt-node-name and --pt-node-ip or ptHost(node_name, ip=127.0.0.1, "
                                "scan_info=False)" % msg)
                sys.exit(-1)
            else:
                logging.warning("%s, pass it as ptHost(node_name, ip=127.0.0.1, scan_info=False)" % msg)
            return
